---
title: "Hvordan land ble påvirket av finanskrisa"
...
#Making a vector for ggvis graph
Location <- as.vector(unique(Merged$Location))
...
Merged %>%
  ggvis(~Time, ~Value) %>%
  filter(Location == eval(input_select(
    choices = Location,
    selected = "NOR",
    label = "Country"))) %>%
  layer_paths()
